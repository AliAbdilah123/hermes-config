...
Lists registration profiles that match the specified compartment or
profile OCID. Filter the list against a variety of criteria including
but not limited to its name, status, vendor name, and architecture
type.
...
The OCID of the registration profile. A filter used to return the
specified profile.

--profile-type [text]

A filter to return registration profiles that match the given profile
type.

Accepted values are:

   GROUP, LIFECYCLE, SOFTWARESOURCE, STATION, UBUNTU_STANDALONE, WINDOWS_STANDALONE
