...
Creates an Autonomous VM cluster for Exadata Cloud@Customer. To create
...
The user-friendly name for the Autonomous VM cluster. The name does
...
the recommended model and OCPU compute model is legacy.
...
The list of OCIDs of the Db servers. This is a complex type whose
...
--distribution-algorithm [text]

The distribution algorithm used for the Autonomous VM cluster.

Accepted values are:

   DISTRIBUTION_OPTIMIZED, RESOURCE_OPTIMIZED
...
the Autonomous VM cluster. If false, database backup on local Exadata
storage is not available in the Autonomous VM cluster.

--is-mtls-enabled [boolean]

Enable mutual TLS(mTLS) authentication for database while provisioning
a VMCluster. Default is TLS.
...
--sga-percentage [float]

Percentage of ECPU memory allocated for SGA(System Global Area).

--time-zone [text]

The time zone to use for the Autonomous VM cluster. For details, see
DB System Time Zones.
