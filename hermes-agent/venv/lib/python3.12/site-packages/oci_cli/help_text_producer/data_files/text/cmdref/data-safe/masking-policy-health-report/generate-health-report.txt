...
=====
   ...
   oci data-safe masking-policy-health-report generate-health-report [OPTIONS]
...
The type of health check. The default behaviour is to perform all
health checks.
...
The OCID of the compartment where the health report resource should be
created.
...
The tablespace that should be used to estimate space. If no tablespace
is provided, the DEFAULT tablespace is used.

--target-credentials [complex type]
...
The OCID of the target database to use for the masking policy health
check. The targetId associated with the masking policy is used if this
is not passed.
...
"--wait-for-state" SUSPENDED would return on whichever lifecycle state
...
security policies before trying the examples.
       ...
       oci data-safe masking-policy-health-report generate-health-report --masking-policy-id $masking_policy_id
