...
paravirtualized devices using VirtIO drivers. * *ACCELERATEDPV* - VM
instances launch with accelerated paravirtualized networking type. *
*CUSTOM* - VM instances launch with custom configuration settings
specified in the *LaunchOptions* parameter.

Accepted values are:

   ACCELERATEDPV, CUSTOM, EMULATED, NATIVE, PARAVIRTUALIZED
...
--wait-for-state" PROVISIONING "--wait-for-state" DELETED would return
