...
The user-friendly name for the VM cluster. The name does not need to
be unique.
...
The list of Db server. This is a complex type whose value must be
...
--exascale-db-storage-vault-id [text]
...
the VM cluster. If false, database backup on local Exadata storage is
not available in the VM cluster.

--is-sparse-diskgroup-enabled [boolean]

If true, the sparse disk group is configured for the VM cluster. If
false, the sparse disk group is not created.
...
The number of OCPU cores to enable for the VM cluster. Only one
...
The time zone to use for the VM cluster. For details, see DB System
Time Zones.
...
security policies before trying the examples.
       ...
       oci db vm-cluster create --compartment-id $compartment_id --cpu-core-count $cpu_core_count --display-name $display_name --exadata-infrastructure-id $exadata_infrastructure_id --gi-version $gi_version --ssh-public-keys file://ssh-public-keys.json --vm-cluster-network-id $vm_cluster_network_id
