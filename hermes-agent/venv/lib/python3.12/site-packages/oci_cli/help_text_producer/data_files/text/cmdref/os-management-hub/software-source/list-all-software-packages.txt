...
Lists software packages available through the OS Management Hub
service.  Filter the list against a variety of criteria including but
not limited to its name.


Usage
=====

   oci os-management-hub software-source list-all-software-packages [OPTIONS]
...
A filter to return software packages that match the given
architecture.

Accepted values are:

   AARCH64, I386, I686, NOARCH, OTHER, SRC, X86_64
...
Indicates whether to list only the latest versions of packages, module
streams, and stream profiles.
...
The field to sort packages by. Only one sort order may be provided.
Default order for displayName is ascending. If no value is specified
