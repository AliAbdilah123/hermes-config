...
Starts the provisioning of a new stream. The stream will be created in
the given compartment id or stream pool id, depending on which
parameter is specified. Compartment id and stream pool id cannot be
specified at the same time. To track the progress of the provisioning,
you can periodically call GetStream. In the response, the
...
--retention-in-hours [integer]

The retention period of the stream, in hours. Accepted values are
between 24 and 168 (7 days). If not specified, the stream will have a
...
security policies before trying the examples.
       ...
       oci streaming admin stream create --name $name --partitions $partitions
