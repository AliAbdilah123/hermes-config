...
--new-admin-password [text]

A new strong password for SYS, SYSTEM, and the plugbable database
ADMIN user. The password must be at least nine characters and contain
at least two uppercase, two lowercase, two numeric, and two special
characters. The special characters must be _, #, or -.

--new-tde-wallet-password [text]

The new password to open the TDE wallet. The password must be at least
nine characters and contain at least two uppercase, two lowercase, two
numeric, and two special characters. The special characters must be _,
#, or -.

--old-tde-wallet-password [text]
...
--wait-for-state" PROVISIONING "--wait-for-state" FAILED would return
on whichever lifecycle state is reached first. If timeout is reached,
a return code of 2 is returned. For any other error, a return code of
1 is returned.
...
--zero-data-loss-enabled [boolean]

Enable Zero Data loss feature
...
security policies before trying the examples.
       ...
       oci db database update --database-id $database_id
