...
Flag indicating whether historical metrics are collected for assets,
originating from this asset source.

--are-realtime-metrics-collected [boolean]

Flag indicating whether real-time metrics are collected for assets,
originating from this asset source.
...
The OCID of the discovery schedule that is going to be attached to the
created asset.

--display-name [text]

A user-friendly name for the asset source. Does not have to be unique,
and it’s mutable. Avoid entering confidential information. The name is
generated by the service if it is not explicitly provided.
...
Flag indicating whether cost data collection is enabled for assets,
originating from this asset source.
...
Endpoint for OLVM asset discovery and replication in the form of
"`https://<host>:<port>`"

--replication-credentials [complex type]
