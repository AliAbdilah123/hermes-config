"update-autonomous-database-okv-key-details"
********************************************
...
Updates one or more attributes of the specified Autonomous AI
Database. See the UpdateAutonomousDatabaseDetails resource for a full
...
=====
   ...
   oci db autonomous-database update-autonomous-database-okv-key-details [OPTIONS]
...
--encryption-key-certificate-directory-name [text]

OKV certificate directory name

--encryption-key-directory-name [text]

OKV wallet directory name

--encryption-key-okv-kms-key [text]

UUID of OKV KMS Key

--encryption-key-okv-uri [text]
...
Guard enabled or Access Control is disabled. *TRUE* if the Autonomous
AI Database has Data Guard and Access Control enabled, and the
Autonomous AI Database uses the primary’s IP access control list (ACL)
for standby. *FALSE* if the Autonomous AI Database has Data Guard and
Access Control enabled, and the Autonomous AI Database uses a
different IP access control list (ACL) for standby compared to
primary.
...
The number of CPUs to be made available to the Autonomous AI
Database.<br> For Autonomous AI Databases on Dedicated Exadata
...
attached to the database. For Autonomous AI Databases on dedicated
...
New name for this Autonomous AI Database. For Autonomous AI Databases
on Dedicated Exadata Infrastructure, the name must begin with an
alphabetic character, and can contain a maximum of eight alphanumeric
characters. Special characters are not permitted. For Autonomous AI
...
value for this parameter. When creating an Autonomous AI Database, if
this parameter is not specified, the default value is *OLTP*.
...
not have to be unique. The display name can only be updated for
Autonomous AI Databases using dedicated Exadata Infrastructure. This
parameter may not be updated in parallel with dbVersion.

--encryption-key-certificate-id [text]
...
Indicates whether auto scaling is enabled for the Autonomous AI
Database CPU core count. Setting to *TRUE* enables auto scaling.
Setting to *FALSE* disables auto scaling. The default value is *TRUE*.
Auto scaling is only available for Autonomous AI Database Serverless
instances.
...
** Deprecated. ** Indicates whether the Autonomous AI Database has a
local (in-region) standby database. Not applicable when creating a
cross-region Autonomous Data Guard associations, or to Autonomous AI
...
an Autonomous AI Database Serverless instance (local and cross-region)
, see About Standby Databases

To enable cross-region Autonomous Data Guard on an Autonomous AI
Database Serverless instance, see Enable Autonomous Data Guard.
...
True if user wants to disable Autonomous AI Database scheduled upgrade
to 26ai.

--is-disconnect-peer [boolean]

If true, this disconnects the Autonomous AI Database from its peer.
After the disconnect completes, the Autonomous AI Database works
permanently as a standalone database.

**Warning:** A disconnected standby is no longer part of the disaster
recovery configuration. Operations and restrictions that apply to a
connected standby do not apply in the same way after the database has
been disconnected.

To disconnect a cross region standby, also provide the OCID of the
standby database in the *peerDbId* parameter.
...
Indicates whether the Autonomous AI Database has a local (in-region)
...
True if user wants to schedule Autonomous AI Database upgrade to the
...
--local-adg-resource-pool-leader-id [text]

The OCID of a dedicated resource pool leader Autonomous AI Database in
the same region, that is required when local Autonomous Data Guard is
enabled for a dedicated resource pool member using the parameter
*isLocalDataGuardEnabled*. This field applies only to dedicated
resource pool members, and the specified leader must be different from
the primary’s leader. Local Autonomous Data Guard can be enabled only
if more than one dedicated resource pool exists in the region.
...
The number of OCPU cores to be made available to the Autonomous AI
Database.

For Autonomous AI Databases on Dedicated Exadata Infrastructure, you
can specify a fractional value for this parameter. Fractional values
are not supported for Autonomous AI Database Serverless instances. For
Autonomous AI Database Serverless instances, this parameter is not
used.
...
Indicates the Autonomous AI Database mode. The database can be opened
...
When disconnecting a cross-region standby, specify the standby
database OCID in this parameter together with *isDisconnectPeer=true*.

--permission-level [text]

The Autonomous AI Database permission level. Restricted mode allows
...
automatically being refreshed with data from the source Autonomous AI
Database.
...
--time-maintenance-pause-until [datetime]

The date until which Autonomous AI Database maintenance is temporarily
paused.
...
Autonomous AI Database refreshable clone. This value controls only the
...
The date and time the Autonomous AI Database scheduled to upgrade to
26ai.
...
--vanity-url-details [complex type]
...
security policies before trying the examples.
       ...
       export encryption_key_certificate_directory_name=<substitute-value-of-encryption_key_certificate_directory_name> # https://docs.cloud.oracle.com/en-us/iaas/tools/oci-cli/latest/oci_cli_docs/cmdref/db/autonomous-database/update-autonomous-database-okv-key-details.html#cmdoption-encryption-key-certificate-directory-name
       export encryption_key_directory_name=<substitute-value-of-encryption_key_directory_name> # https://docs.cloud.oracle.com/en-us/iaas/tools/oci-cli/latest/oci_cli_docs/cmdref/db/autonomous-database/update-autonomous-database-okv-key-details.html#cmdoption-encryption-key-directory-name
       export encryption_key_okv_kms_key=<substitute-value-of-encryption_key_okv_kms_key> # https://docs.cloud.oracle.com/en-us/iaas/tools/oci-cli/latest/oci_cli_docs/cmdref/db/autonomous-database/update-autonomous-database-okv-key-details.html#cmdoption-encryption-key-okv-kms-key
       export encryption_key_okv_uri=<substitute-value-of-encryption_key_okv_uri> # https://docs.cloud.oracle.com/en-us/iaas/tools/oci-cli/latest/oci_cli_docs/cmdref/db/autonomous-database/update-autonomous-database-okv-key-details.html#cmdoption-encryption-key-okv-uri
       ...
       oci db autonomous-database update-autonomous-database-okv-key-details --autonomous-database-id $autonomous_database_id --encryption-key-certificate-directory-name $encryption_key_certificate_directory_name --encryption-key-directory-name $encryption_key_directory_name --encryption-key-okv-kms-key $encryption_key_okv_kms_key --encryption-key-okv-uri $encryption_key_okv_uri
