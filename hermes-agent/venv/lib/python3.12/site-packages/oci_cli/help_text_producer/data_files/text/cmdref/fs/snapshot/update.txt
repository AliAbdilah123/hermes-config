...
=====
   ...
   oci fs snapshot update [OPTIONS]
...
The UTC time when this snapshot will be deleted. To remove the
expiration time, set this field to the minimum date-time value using
Date(0).
...
security policies before trying the examples.
       ...
       snapshot_id=$(oci fs snapshot create --file-system-id $file_system_id --name $name --query data.id --raw-output)

       oci fs snapshot update --snapshot-id $snapshot_id
