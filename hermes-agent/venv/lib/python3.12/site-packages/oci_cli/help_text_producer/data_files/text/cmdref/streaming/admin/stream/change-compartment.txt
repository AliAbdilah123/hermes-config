...
is checked against ETag values of the resource. The stream will also
be moved into the default stream pool in the destination compartment.
...
security policies before trying the examples.
       ...
       export compartment_id=<substitute-value-of-compartment_id> # https://docs.cloud.oracle.com/en-us/iaas/tools/oci-cli/latest/oci_cli_docs/cmdref/streaming/admin/stream/change-compartment.html#cmdoption-compartment-id

       stream_id=$(oci streaming admin stream create --name $name --partitions $partitions --query data.id --raw-output)

       oci streaming admin stream change-compartment --compartment-id $compartment_id --stream-id $stream_id
