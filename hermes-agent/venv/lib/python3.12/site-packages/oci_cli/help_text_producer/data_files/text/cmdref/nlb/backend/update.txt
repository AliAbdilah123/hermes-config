...
=====
   ...
   oci nlb backend update [OPTIONS]
...
The name of the backend server to update. If the backend was created
with an explicitly specified name, that name should be used here. If
the backend was created without explicitly specifying the name, but
was created using ipAddress, this is specified as <ipAddress>:<port>.
If the backend was created without explicitly specifying the name, but
was created using targetId, this is specified as <targetId>:<port>.

Example:

   10.0.0.3:8080` or `ocid1.privateip..oc1.<var>&lt;unique_ID&gt;</var>:8080
...
marked “isDrain” stop receiving new connections but will continue to
receive traffic on existing connections until the connection is
...
information about network load balancer policies, see Network Load
Balancer Policies.
...
security policies before trying the examples.
       ...
       export backend_name=<substitute-value-of-backend_name> # https://docs.cloud.oracle.com/en-us/iaas/tools/oci-cli/latest/oci_cli_docs/cmdref/nlb/backend/update.html#cmdoption-backend-name
       export backend_set_name=<substitute-value-of-backend_set_name> # https://docs.cloud.oracle.com/en-us/iaas/tools/oci-cli/latest/oci_cli_docs/cmdref/nlb/backend/update.html#cmdoption-backend-set-name
       ...
       oci nlb backend update --backend-name $backend_name --backend-set-name $backend_set_name --network-load-balancer-id $network_load_balancer_id
