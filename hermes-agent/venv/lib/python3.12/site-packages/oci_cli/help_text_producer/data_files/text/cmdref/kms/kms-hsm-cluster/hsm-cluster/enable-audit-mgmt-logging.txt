...
EnableAuditLogging / CreateHsmCluster are now in place. If the cluster
is in WAITING_FOR_CUSTOMER, and validation is successful, status is
progressed and a work request is started. If the cluster has already
progressed past that gate, the call is idempotent and returns 202 with
the current cluster representation.


Usage
=====

   oci kms kms-hsm-cluster hsm-cluster enable-audit-mgmt-logging [OPTIONS]
...
security policies before trying the examples.
       ...
       oci kms kms-hsm-cluster hsm-cluster enable-audit-mgmt-logging --hsm-cluster-id $hsm_cluster_id
