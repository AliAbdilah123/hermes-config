...
Lists the BYOL Allocations in the specified compartment. The list can
be filtered by display name or availability domain.


Usage
=====

   oci ocvs byol-allocation list [OPTIONS]
...
--available-units-greater-than-or-equal-to [float]

A filter to return only resources whose availableUnits greater than or
...
A filter to return only resources whose lifecycle state matches the
given value.
...
A filter to return only resources whose softwareType matches the given
value.

Accepted values are:

   AVI_LOAD_BALANCER, VCF, VDEFEND, VSAN

--sort-by [text]

The field to sort by. You can provide one sort order (*sortOrder*).
Default order for *timeTermStart* is descending. Default order for
*timeCreated* is descending. Default order for *displayName* is
...
Accepted values are:
   ...
   displayName, timeCreated, timeTermStart
...
security policies before trying the examples.
       ...
       export compartment_id=<substitute-value-of-compartment_id> # https://docs.cloud.oracle.com/en-us/iaas/tools/oci-cli/latest/oci_cli_docs/cmdref/ocvs/byol-allocation/list.html#cmdoption-compartment-id

       oci ocvs byol-allocation list --compartment-id $compartment_id
