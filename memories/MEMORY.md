...
§
Local verification for completed projects: backend API at 127.0.0.1:8080 before nginx; public URL already works (http://168.110.213.104/projects/demo/).
§
Default project conventions unless overridden: frontend = TypeScript + React via Vite, backend = Go HTTP server, database = SQLite, routes at /api/v1/* plus /projects/<projectName>/api/v1/*, public access at http://168.110.213.104/projects/<projectName> via nginx.
§
...
§
Default project conventions (override only when asked): TypeScript + React via Vite on the frontend, Go HTTP server backend with port 8080, SQLite database, public exposure at http://168.110.213.104/projects/<projectName> via nginx.
§
Public access in this environment is http://168.110.213.104/projects/<projectName> via nginx. Project files should be owned by nginx:nginx and placed under /usr/share/nginx/html/projects/<projectName>.
§
