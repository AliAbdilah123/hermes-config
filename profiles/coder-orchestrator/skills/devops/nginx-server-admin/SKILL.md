...
- **Shell expansion can silently corrupt generated nginx blocks**: Do not put nginx variables such as `$uri` inside a double-quoted shell command or unquoted heredoc; the shell may expand them before writing the config, producing rules like `try_files / /projects/app/index.html` and redirect loops. Prefer a single-quoted Python/Perl payload, a quoted heredoc, or escape every `$`. Immediately read back the inserted block before `nginx -t` and verify it still contains literal `$uri`.
- **Reload may leave old workers serving a repaired route inconsistently**: After fixing an internal-redirection loop, test with the intended `Host` header as well as `localhost`. If `nginx -t` and reload succeed but localhost and public-host results disagree or logs still name the old rule, perform a bounded `systemctl restart nginx`, then re-probe localhost, origin IP, origin with `--resolve`, and the public URL. Treat this as a reload boundary, not an application defect.
- **Dist not auto-deployed to nginx**: After `npm run build` / `npx vite build` completes, the build output sits in the project's local `dist/` directory. A git push alone does not update nginx-served sites. You MUST manually copy it to the served directory, preferably with deletion so removed hashed assets do not linger: `sudo rsync -a --delete dist/ /var/www/html/projects/<slug>/`. If the user says they "don't see" a change you know you made, first compare the live index asset hash with the built one: `curl -fsSL https://<domain>/ | grep -o 'assets/index-[^\"]*\\.js' | head -1` and `grep -o 'assets/index-[^\"]*\\.js' dist/index.html | head -1`.
- **Domain deploys use the same directory now**: Domains are served from `/var/www/html/projects/<slug>/` via the single-build + sub_filter pattern. Deploying to that one directory updates BOTH the IP path and the domain. If a domain existed before this change, it may have had a stale `/var/www/html/<slug>/` copy — remove it. The #1 stale-domain suspect is an out-of-date `/var/www/html/projects/<slug>/` — check the asset hash or `md5sum` against the source `dist/index.html`.
- **Cloudflare caches old hashed asset paths**: If the domain HTML updates but the referenced Vite hash is already cached as the old bundle, or the newly referenced asset URL returns HTML due to a stale fallback cache, purge Cloudflare when credentials exist. Without a token, use a one-off cache-bust: copy the built main bundle to a fresh name (for example `assets/index-<commit>.js`) and replace that filename in the deployed `index.html`, then verify `curl -sI https://<domain>/assets/index-<commit>.js` returns `Content-Type: application/javascript` and the live index references the new name. This is a hotfix for proxied nginx sites with `max-age=14400`, not a source-code change.
...
- **Relative slash redirects can downgrade public HTTPS behind Cloudflare**: An exact nginx rule such as `location = /projects/<slug> { return 301 /projects/<slug>/; }` may emit an absolute `Location: http://<domain>/...` when Cloudflare connects to the origin over HTTP, even though the visitor requested HTTPS. For a canonical public HTTPS hostname, return the explicit external URL instead: `return 301 https://<domain>/projects/<slug>/;`. Verify both forms publicly with `curl -sSIk https://<domain>/projects/<slug>` and require an HTTPS `Location`, then verify the trailing-slash destination returns the app.
