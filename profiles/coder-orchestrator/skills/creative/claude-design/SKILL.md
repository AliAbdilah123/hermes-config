...
User-specific review-document deliverable:

- When producing any document for this user to review — plans, PRDs, specs, audits, migration docs, decision records, or similar — also produce a styled HTML artifact, not only markdown/plain text.
- Store the canonical HTML under the relevant project path at `<project path>/docs/<name>.html` when a project path exists.
- Publish it at `http://<publicip>/prd/<name>.html` by symlinking from the web server PRD directory to the docs file; do not leave it only in `/home/ubuntu` or `.hermes/plans`.
- Verify the docs file, symlink, nginx/web-server route, and public HTTP 200 response before finalizing.
- End the final response with the public review-document link when the document is the main deliverable.
- See `references/prd-html-publication.md` for the concrete publication workflow and verification checklist.
...
- See `references/prd-html-publication.md` for the concrete PRD publication workflow and verification checklist; for plans, also check the `plan` skill's `references/review-doc-html-publication.md` workflow.
...
- HTML slide decks
- PRD-style product concept documents and product requirements artifacts
- motion studies
- onboarding flows
- dashboard concepts
...
For this user, product/application ideation requests should default to a PRD-styled HTML artifact rather than a plain-text strategy memo when the user asks for an app/project concept, product requirements, MVP shape, or similar product-planning output. If the user says “continue” after such a draft, continue by expanding the same HTML artifact with deeper sections such as UX flows, screen inventory, API surface, business rules, non-functional requirements, implementation stack, and acceptance criteria.
