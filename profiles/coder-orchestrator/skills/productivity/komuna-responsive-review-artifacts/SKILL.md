...
- `references/discovery-colorful-carousel-card-rail-prototype.md` — iterative Discovery prototype refinement: one-at-a-time accessible hero carousel, whole-card vertical links, proportional density reduction, three-card desktop rows, conditional mobile rail chevrons, current-branch topbar fidelity, verified-data-only mockups, and shared topbar/footer theme surfaces.
- `references/discovery-colorful-carousel-card-rail-prototype.md` — approved-direction refinement checklist: one-program autoplay carousel, whole-card vertical links, mobile snap rails with conditional chevrons, EN/ID placeholder controls, and complete light/dark component parity while preserving stacked merchandising sections.
