...
- **Provider analytics must fan out by `post_targets`, not assume Instagram**: The shared `posts` metric columns (`likes`, `comments`, `reach`, `impressions`, `saves`, `shares`) drive both the analytics page and post-detail Performance panel. When adding Facebook/Threads analytics, do not change the frontend first unless labels/UX truly differ; add a provider-specific backend refresher and wire it into every refresh path: `analyticsRefresh` in `analytics.go`, `internal/posts.Handler.refreshMetrics`, and `internal/sync.Handler.Sync`. Keep provider limitations explicit: Facebook has no Instagram-style `saves`; Threads maps `replies`→comments, `views`→reach/impressions fallback, and `reposts+quotes`→shares. Because production SQLite uses `SetMaxOpenConns(1)`, collect post IDs and close `Rows` before calling provider refreshers inside sync loops.

- **Analytics platform filtering needs target data plus derived summaries**: The analytics overview shape (`analyticsPosts` in `utils.go`) must include each post's `targets` from `post_targets`; otherwise the frontend cannot distinguish Facebook-only, Instagram-only, Threads-only, and cross-platform posts. For whole-page analytics filters, filter posts by requiring every selected platform to exist in `post.targets` and recompute displayed overview metrics from the filtered posts (`likes + comments + saves + shares`, reach, impressions, post count) so cards/chart/table/PDF stay consistent. Keep the backend date/account query unchanged unless provider-specific SQL filtering is actually needed.
...
- **Photo crop image nearly invisible in dark mode**: Inspect `PhotoCropModal.tsx` plus `globals.css` before changing crop math. The global `.dark [style*="rgb(255..."]` fallback is intentionally broad and can interact badly with media controls; keep the actual image preview scoped and isolated. Use a class on the preview container (for example `.photo-crop-surface`) with `isolation: isolate`, a stable dark media background, and explicitly reset the child image to `filter: none !important`, `opacity: 1 !important`, and `mix-blend-mode: normal !important`. Do not touch drag/resize/pan logic unless reproduction proves it is broken. Verify with `pnpm typecheck`, `pnpm build`, grep built CSS/PhotoCropModal bundle for the scoped marker, deploy, and confirm the JS asset returns `application/javascript`.
...
- **Photo crop dark/blank area after CSS fix requires layer-by-layer RCA**: If a user says the crop bug persists after hard refresh, do not apply another CSS fix first. Prove whether the dark area is in the uploaded pixels, crop rectangle/canvas output, preview rendering, or theme CSS. Check source image metadata/pixels, `ctx.drawImage` inputs, generated file pixels, preview CSS (`overflow`, `object-fit`, `transform`, `filter`, `opacity`, `mix-blend-mode`), parent containers, and light vs dark mode. If the crop selection already contains a large dark/empty region and canvas output preserves it, the root is crop geometry/content bounds, not theme CSS. See `references/photo-crop-root-cause-triage.md`.
...
- **Platform-aware New Post image crop planning/implementation**: When planning or implementing image crop rules for New Post, inspect `CreatePostPage.tsx`, `PhotoCropModal.tsx`, and `internal/posts/handler.go` first. Centralize platform/post-type ratio rules in a small frontend requirements module, keep New Post crops ratio-locked (no `free` preset unless explicitly approved), add `9:16` for stories, treat Instagram rules as the strictest cross-posting constraint, and preserve cropper mobile/dark-mode fixes. For implementation details and verification commands, see `references/platform-aware-image-crop-planning.md` and `references/platform-aware-image-crop-implementation.md`.

- **Mobile crop panning scrolls the page / feels laggy**: Fix gesture ownership at the crop modal boundary. Apply `touchAction: "none"`, `overscrollBehavior`, and `userSelect: "none"` to the actual crop surface/box/handles/timeline, not only the backdrop; call `preventDefault()` + `stopPropagation()` on pointer down/move/up/cancel; capture pointers on the stable container ref; lock body scroll while mounted; throttle image crop box updates with `requestAnimationFrame`. See `references/mobile-crop-gesture-handling.md`.
