...
- **Email verification implementation gates real actions**: Email/password signup should store `email_verified=0`, queue verification after session creation, verify through a frontend `/auth/verify-email?token=...` page that calls the backend then refreshes session, and gate restricted server-side actions such as post creation and social OAuth start with `EMAIL_NOT_VERIFIED`. See `references/email-verification-flow-implementation.md` for the compact implementation and verification checklist.
...
- **Photo crop multi-image state must be keyed by File identity**: If switching between uploaded crop thumbnails makes the crop/result shrink or accumulate transforms — especially when the same source image is uploaded multiple times — inspect `PhotoCropModal.tsx` state persistence before changing crop math. Do not key crop memory by `name:size:lastModified`; duplicate uploads can share that key. Use `WeakMap<File, CropState>` and persist normalized box + preset/output fields per exact `File` object. Thumbnail removal belongs in the parent pending queue: removing the selected image should select the nearest remaining queued file or clear the modal if none remain. See `references/photo-crop-independent-file-state.md` for the compact RCA/fix/verification pattern.
