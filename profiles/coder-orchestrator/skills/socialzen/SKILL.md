...
- See `references/meta-least-privilege-completion.md` when completing an approved Meta audit across OAuth, backend routes/calls, frontend onboarding, legal copy, provider exclusion, deployment, and a post-implementation audit update that separates shipped code from clean-token/Meta Dashboard proof.
- See `references/meta-permission-source-to-feature-reconciliation.md` when an audit says a provider capability is deferred/absent but dormant Graph helpers or shared UI controls still imply it; reconcile OAuth, invocation paths, platform-gated UI, legal copy, tests, and audit wording, and run the canonical verification command literally when required.
- See `references/threads-permission-source-audit.md` when auditing Threads permissions against source. It separates requested OAuth scopes, product surfaces, concrete Graph API calls, blocked reviewer paths, and least-privilege classifications; it also records pitfalls such as aggregate reply metrics not proving reply-reading access.
- See `references/public-legal-pages.md` when implementing or revising SocialZen Terms, Privacy, Security, or Data Deletion pages. It covers the shared public-page shell, basename-aware routes, metadata/cross-links, production-claim accuracy gate, focused tests, deployment checks, and desktop/mobile visual QA.
- See `references/legal-consent-and-current-terms-enforcement.md` when integrating those documents into signup, OAuth signup, authenticated access, the shared landing footer, or Settings → Account & Data. It covers versioned server-side acceptance, dual migrations, an app-shell gate plus API enforcement, a non-dismissible modal, preserved informational deletion guidance, focused tests, deployment ordering, and precise reporting when unrelated suite failures exist.
- See `references/safe-account-deletion-phase1.md` when implementing account deactivation, 30-day restoration, or permanent deletion. It covers the durable state machine, dual migrations, all-login account-state gates, atomic deactivation, no-auto-resume restoration, explicit legacy-dependent cleanup, lifecycle notifications, UI confirmation, TDD, and claim boundaries.
- See `references/account-data-management-phases-1-4.md` when Account & Data work spans deletion/restoration, asynchronous private exports, advanced export controls, import-ready archive contracts, and safe provider disconnection. It covers the shared state/version boundary, Google re-auth equivalence, encrypted/grant-based delivery, warning/expiry/part behavior, connection impact handling, and focused-vs-full-suite verification discipline.
- See `references/account-data-management-completion-hardening.md` when those phases already exist but a revised-PRD audit exposes finishing gaps. It covers provider-specific revocation/enrichment warnings, removing fictitious workspace ownership, dedicated versioned export keys, honest ZIP64/part evidence, responsive cancellation, durable skipped/retryable lifecycle delivery, deletion-safe outbox snapshots, edited retry filters, specific Google re-auth errors, import-ready validation, policy decisions, and the no-commit/no-deploy completion gate.
- See `references/account-data-pending-deletion-restoration-delivery.md` when polishing an implemented lifecycle: pending-deletion disabled/preserved-data disclosures, persisted deletion date and remaining days, restoration notifications and reconnect requirements, direct available-archive actions in Notification Center, provider metadata preservation, and deployment startup-config preflight.
...
- See `references/analytics-review-completion-audit.md` before declaring an approved Analytics review fully implemented. Audit every numbered requirement across backend, DTOs, UI, sparse states, configured timezone, and PDF; completing only ranking/thumbnails or the refresh card is not proof that the whole review is done.
- See `references/analytics-refresh-result-card-ux.md` when implementing the approved Analytics refresh summary/details card, safe destination-level reasons/actions, evidence-based unavailable explanations, or renaming What Worked to Content Performance Insights. It includes the response/UI contract, account scoping, TDD checks, and visual-verification boundary.
- See `references/analytics-refresh-sequential-provider-load.md` when the in-page Analytics refresh appears to crash, freeze, or stay heavy after a frontend loading-state fix. It covers target/request counting, serialized Meta API latency, unsupported insight metrics, bounded-concurrency constraints, and real end-to-end verification before declaring the bug fixed.
- See `references/analytics-background-job-tdd-handoff.md` when implementing the approved durable analytics refresh queue. It enforces acceptance-row tracking, request-independent runner extraction, migration of synchronous tests to runner tests, unavailable/notification completion, focused-vs-full verification, and honest partial-completion reporting.
- See `references/analytics-post-detail-date-safety.md` when Analytics post detail crashes on sparse historical/provider data. It covers the `date-fns` invalid-date render exception, boundary fallback, focused RED/GREEN regression, and the distinction between refresh-job stability and DTO render safety.
- See `references/instagram-reconnect-sync-resolution.md` when published Instagram targets retain a disconnected local account row after reconnect. It covers exact-first lookup, safe same-user/provider `ig_user_id` fallback shared by comments and analytics, ambiguity fail-closed behavior, pagination/field checks, and focused TDD verification without adding OAuth scopes.
- See `references/comment-manual-refresh-provider-status.md` when adding a visible comment refresh action. It covers the separate synchronous refresh endpoint, provider-specific ok/empty/error/unavailable statuses, cache-first async GET preservation, all-page reload ordering, UI messaging, and strict RED/GREEN checks.
- See `references/instagram-empty-comments-permission-mismatch.md` when Instagram returns HTTP 200 with `data:[]`; compare the media owner and `comments_count`, verify the real media/account IDs and documented scopes, distinguish Facebook comments, and surface a reconnect/Advanced Access error when the provider count is nonzero.
- See `references/vite-stale-chunk-reload-root-cause.md` when an authenticated action appears to crash or reload the SPA. It requires console/CDP/network evidence before blaming the preload handler, shows how stale hashed chunks can receive misleading HTTP 200 SPA HTML fallbacks after `rsync --delete`, and defines safe cache/deployment remediation without blindly removing recovery.
...
- See `references/notification-center-planning.md` when planning or implementing persistent cross-domain notifications. It covers the minimal SQLite snapshot/dedupe model, committed producer boundaries, exact target-level analytics milestones, Settings detail/deep-link routes, polling, cursor pagination, accessibility, TDD, and single-connection SQLite pitfalls.
- See `references/standalone-settings-page-shell-parity.md` when a dedicated Settings route bypasses `SettingsPage` and loses the shared quick navigation or page geometry. It covers route precedence, shared-menu extraction, active-state/navigation tests, left-aligned `max-w-[1040px]` parity, and clean-worktree delegation safety.
- See `references/notification-center-completion-audit.md` before claiming an approved Notification Center plan is fully implemented. Audit every plan and acceptance row with file-line, test, and authenticated production evidence; distinguish available helper APIs from producers wired at real committed domain boundaries, and report Partial/Missing items explicitly. For the durable remediation pattern—processed markers for preference suppression, exhaustive reconciliation, subscription outbox retry, archived badge deltas, decoded route-shape allowlisting, and hourly token-expiry maintenance—see `references/notification-center-reconciliation-and-delivery-hardening.md`.
- See `references/notification-center-completion-tdd.md` when implementing the audit's completion fixes. It requires an explicit ten-row checklist, per-fix RED evidence, fresh post-edit builds, and regression shapes for exhaustive publishing reconciliation, archived mark-all-read, deep-link allowlisting, and multi-provider token-expiry warnings.
- See `references/notification-center-five-finding-tdd.md` for the compact five-finding pattern covering durable preference-suppression markers, a retryable subscription notification outbox, active-unread badge semantics for archived history, structural deep-link allowlisting, maintenance cadence separation, and mandatory fresh post-edit verification.
- See `references/notification-settings-history-first-handoff.md` when an approved Settings redesign makes durable notification history primary and moves existing delivery preferences into a final collapsed native disclosure. It covers the minimum UI-only diff, focused hierarchy regression, bell non-regression boundary, nested-card flattening, and clean-worktree deploy/commit pattern for a dirty checkout.
- See `references/notification-center-gap-remediation.md` when a completion audit finds several gaps in an already-shipped Notification Center. It covers preference-suppression tombstoning, exact analytics cooldown/target identity, immediate badge reconciliation, visible mutation failures, real subscription-boundary wiring, and the rule that a partial delegated pass cannot close the remaining audit rows. For concrete regression shapes covering exhaustive reconciliation, cross-provider token expiry, archived mark-all-read, subscription transition dedupe, unread delta events, and strict route allowlisting, also see `references/notification-center-completion-fixes.md`.
- See `references/notification-center-subscription-and-badge-completion.md` for the focused implementation recipe behind subscription plan-transition delivery and frontend mutation synchronization: shared committed payment boundary, old/new plan snapshots, provider-event dedupe, visible errors, unread delta events, and strict RED/GREEN regression shapes.
...
- See `references/multi-account-post-target-selection.md` when the composer must select several connected accounts on one or more platforms. It covers the generic `targets` contract, platform-keyed checkbox state, ownership/active validation, duplicate rejection, backward compatibility, atomic post/target persistence, TDD, and deployment verification.
...
When adding or retiring short-lived/test subscription plans, keep purchase availability, Xendit invoice lookup, period handling, status identity, and frontend ordering in sync. Retirement removes plans from `PlansData()` but preserves legacy `planInfo()`/`PlanPeriod()` behavior for existing rows. See `references/subscription-one-minute-plans.md`.
...
- **Open comment drawer must refresh every provider, retain every page, and surface provider outcomes**: Keep normal GET cache-first and asynchronous, then lightly poll while the drawer is mounted. Add a separate explicit refresh action because a background GET sync cannot return its eventual result: synchronously run Instagram and Facebook sync, return per-provider `ok` / `empty` / `error` / `unavailable` status, and reload all local API pages before replacing state. A successful Graph `data:[]` response is an informational empty result, not a fetch failure; actual HTTP/token/parse failures need safe provider-specific UI messages as well as redacted logs. Preserve separate top-level/reply cursor pagination, auto-expand replies, and use a larger scrollable dialog with an accessible busy refresh control. See `references/all-platform-comment-refresh.md` and `references/comment-manual-refresh-provider-status.md`.
...
- **Landing feature availability must be honest and ordered**: In `pages/landing/LandingFeatures.tsx`, list successfully shipped features first from left to right, followed by unavailable/planned features. Keep availability in feature metadata and render unavailable cards with a clearly muted grayscale/low-opacity treatment plus a subtle inset shadow; retain their explanatory copy. Keep the English and Indonesian `features.cards` arrays in `lib/language.tsx` in exactly the same order as the visual metadata because localized copy is mapped by index. Treat recurring/repeat scheduling, inline comment replies, and AI best-time-to-post as unavailable unless current source and production behavior prove otherwise. Before commit, run fresh frontend typecheck/build through a working repository package-manager entry point, deploy to `/var/www/html/projects/socialzen/`, verify the public page and deployed JS content type, then commit and push.
...
- **Instagram PHOTO is single-image; untouched carousel items must inherit the batch preset**: When Instagram is among the selected targets, cap PHOTO at one image in both composer and CreatePost validation, and direct multiple-image users to CAROUSEL_ALBUM. Preserve Facebook-only multi-photo PHOTO behavior. If an untouched carousel item fails with `Photo N does not match an allowed aspect ratio`, do not weaken validation: its full-frame fallback is preserving the source ratio. Fit that fallback to the active batch preset before canvas materialization, while preserving explicit per-file crop state and order. See `references/instagram-single-photo-and-carousel-ratio-enforcement.md`.
