...
- Run focused backend tests and build, focused frontend tests and production build, then the broad suites. Classify broad-suite failures precisely as changed-feature regressions or pre-existing/unrelated failures; never report “complete” while acceptance criteria remain unverified.
- Do not deploy until real connected Instagram Business and Facebook Page tests cover the supported media combinations and confirm per-target provider records, partial failure, and failed-only retry.
- Final results must distinguish implemented, automated verification passed, broad-suite blockers, deployment status, and real-provider verification still required.
