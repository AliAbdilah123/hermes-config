# Legacy skill package: `release-and-expose-web-app`

This file preserves the former `release-and-expose-web-app` SKILL.md after consolidation into `web-app-delivery`. Relative support-file links have been rewritten to the re-homed files under `web-app-delivery`.
...
1. Keep active coding project source trees under `~/projects/<projectName>` unless the user explicitly chooses another root. If migrating older projects into `~/projects`, stop their systemd/dev processes first, move the source dirs, then update service `WorkingDirectory` paths before restarting.
2. `npm run build` in `frontend/`.
3. Build the Go backend into a persistent binary (for example `/opt/<project>/<project>-server`) and run it via systemd for delivered apps. Use a tracked background `go run .` only for temporary smoke testing; do not leave the final deployment depending on an agent background process.
4. Start the service from the backend working directory so SQLite paths stay stable, e.g. `WorkingDirectory=/home/ubuntu/projects/<project>/backend` and `ExecStart=/opt/<project>/<project>-server`.
5. Verify backend locally: `curl -I -s http://127.0.0.1:<PORT>/api/v1/hello` -> 200 before nginx verification.
...
- Template available: `templates/release-and-expose-web-app-nginx-project-with-prds.conf` contains a known-good single-server config for `/projects/<projectName>/`, `/projects/<projectName>/api/v1/`, root `/api/v1/`, and `/prds/` on Ubuntu-style hosts.
...
- On Ubuntu-style Hermes hosts, the authoritative nginx config may be `/etc/nginx/nginx.conf` including `/etc/nginx/projects/*.conf`, with project assets under `/var/www/html/projects/<projectName>` and PRDs under `/var/www/html/prds`. Inspect the current include/root paths before assuming `/etc/nginx/conf.d` or `/usr/share/nginx/html`.
...
- Moving source directories usually does **not** require nginx alias changes when nginx serves built artifacts from `/var/www/html/projects/...`; check nginx for source-path references anyway, but expect systemd `WorkingDirectory` to be the path that needs updating.
- Self-curling the public IP from the same host may fail due to hairpin/NAT behavior even when local nginx routes work. Treat `127.0.0.1` nginx checks as the primary on-host verification, and only report public-IP timeout as a network-path caveat, not an app failure.
