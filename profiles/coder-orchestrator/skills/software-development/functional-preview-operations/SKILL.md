...
7. Keep executing the verification chain continuously until complete or concretely blocked. Do not repeatedly report “still working,” “next I will,” or imply background activity when no process is running. Use status labels literally: `WORKING` only while a tool/delegate or concrete hands-on step is active; `VERIFYING` only while checks are actively running; `READY FOR REVIEW` only after publication plus exact public E2E; `STOPPED`/`BLOCKED` as soon as execution is inactive. Name the active step. When a background task finishes, resume fresh verification and publication without waiting for the user to say “continue.” If interrupted, state exactly what is complete, what is not, and whether anything is currently executing.
8. Recheck service activity immediately before sending the URL. After backend/frontend changes, do not claim restart or publication until the replacement process is listening and public health plus served asset hashes prove the new artifacts are active. Recheck after any gateway restart or long review delay. If the runtime was started through a chat-owned background process, replace it with the durable service before sharing—even if the process is currently healthy.
9. Record production asset identity before and after to prove isolation.
...
For multi-role approval flows and notification-to-inbox navigation, follow `references/role-context-approval-notification-e2e.md`.
