...
9. Push the feature branch when required, then finish publishing and verifying the preview before reporting completion. A coding-agent completion, commit, passing tests, or the phrase “preparing/publishing the preview” is only intermediate progress—not the deliverable. Distinguish precisely: **implemented locally** (no URL), **published but unverified** (URL exists but is not reviewable), and **publicly verified** (exact public route, isolated API, assets, required role flow, browser DOM/console, and production isolation pass). Only in the third state provide the preview URL and request approval. If publication is blocked, report the concrete blocker rather than promising the URL later.
