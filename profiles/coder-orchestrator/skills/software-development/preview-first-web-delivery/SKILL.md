...
5. Run focused tests and the production build from the final workspace state. Keep unrelated baseline lint failures separate from changed-file verification.
6. Build with the preview's actual public base path when the bundler supports a configurable base (for example, `VITE_BASE=/previews/<slug>/ npm run build`).
...
For backend-dependent previews, follow `references/full-stack-isolated-preview.md` for the loopback API, copied database, Nginx proxy, build variables, and content-level verification pattern.
