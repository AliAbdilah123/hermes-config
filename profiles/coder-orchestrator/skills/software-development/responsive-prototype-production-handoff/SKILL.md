...
- Treat generated Playwright suites as test code requiring review, not proof. Confirm that fixtures authenticate through the real supported public flow or deterministic test setup, that each named route and viewport is actually exercised, and that assertions cover the approved interaction contract. Do not announce authenticated public E2E as the next automatic step until credentials/fixtures and the deployed route are known to support it.
...
- When a user marks up a screenshot, treat the marked pixels and their DOM element as the visual contract. Explicitly distinguish the eyebrow/kicker, primary heading, subtitle, and surrounding container before editing. Do not reinterpret “remove this text” as changing translations, typography, or a nearby heading. If the user says to hide the marked item, remove the rendered element from the exact route rather than merely stripping a prefix from its string.
- After a user rejects a visual interpretation, stop incremental styling guesses. Re-inspect the screenshot, trace the exact route/component, restate the one targeted element, and make the smallest DOM-level correction. Do not present unrelated earlier changes as satisfying the correction.
- If the user approves immediately after that clarification, approval applies to the corrected visual contract—not the previously rejected implementation. Ensure the correction itself is committed, included in the clean integration build, and verified in the route-specific production chunk before removing the preview.
