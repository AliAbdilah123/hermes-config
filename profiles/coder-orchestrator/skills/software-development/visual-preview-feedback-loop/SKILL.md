...
4. **Build for the actual preview mount.** Before building a nested SPA preview, inspect the bundler's configured base and explicitly set it to the exact public preview prefix (including its trailing slash when required). After the build, inspect `dist/index.html` and require every entry script, module preload, and stylesheet URL to begin with that preview prefix; a successful build with production asset URLs will publish a blank or mixed-version page. Publish from the package's real output directory, and let a failed copy stop the deployment command—never print or report a success marker after `rsync`/copy failure.
5. **Render exact public routes.** Exercise each changed route at the relevant viewport. A preview root, HTTP 200, source assertion, or build does not prove a detail route. Before browser QA, fetch the public HTML, extract its entry JS/CSS URLs, and require those exact assets to return 200 with JavaScript/CSS MIME types from the preview namespace—not from production.
