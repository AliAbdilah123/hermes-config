...
- A paid database row is not proof that the user-visible return flow works. Before declaring READY, replay the **exact provider-issued return URL** (including host, mount path, and query string) in an authenticated public browser, confirm from server logs which API/database receives the request, and assert the error UI is absent. Do not substitute a manually corrected URL.
- Never infer the affected purchase or account from an earlier checkout, familiar email, existing auth fixture, or nearby paid invoice. Correlate the user's latest browser request by timestamp/IP/session to the checkout log's `purchase_id` and `user_id`, join the purchase through membership to the actual account, and verify the provider invoice external ID matches. If multiple purchases exist, list and disambiguate them before recovery.
- Do not report entitlements delivered until the **actual buyer's** wallet/entitlement endpoint and authenticated public UI show them. Record per-purchase entitlement counts before and after, and verify rows belong to that buyer's membership—not merely that some entitlements exist in the database.
- For recovery of an already-created invoice, distinguish the code fix for future invoices from remediation of the old provider-issued URL. If the old URL targets another environment, reproduce that exact URL first, then add only a narrowly scoped compatibility bridge. Verify the bridge against the actual purchase ID at origin and public edge, including its final HTTPS `Location`; broad regexes, a different purchase ID, or a corrected synthetic URL are not proof.
...
See `references/xendit-go-sqlite.md` for a concrete Xendit/Go/SQLite checklist, `references/pending-expiry-and-resume.md` for provider-truth expiry, reconciliation scheduling, owner-only invoice links, and purchase-surface tests, `references/preview-safe-local-checkout.md` for explicitly gated providerless preview checkout, strict TDD, public-chain gates, and verification-script hygiene, and `references/manual-transfer-payments.md` for multi-tenant manual-transfer receipt review, filesystem/DB crash windows, resubmission intent, and bearer-auth receipt delivery.