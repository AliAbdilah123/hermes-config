...
4. OAuth URL/provider-flow checks.
   ...
   - **Threads long-lived token exchange**: Threads short-lived tokens expire in 1 hour. Exchange with `grant_type=th_exchange_token` at `GET /v1.0/access_token` — same pattern as Instagram/Facebook. Without this, Threads connections die after one hour.
   - **Threads accounts saved to wrong table**: If `oauthCallbackGeneric` inserts into `instagram_accounts` for all providers, Threads accounts land there with `provider='threads'` and no token. Add a provider guard to save Threads to `threads_accounts` instead, then return early.
   - **Migration backfill creates duplicate post_targets**: A `migrate()` entry using `INSERT OR IGNORE INTO post_targets ... SELECT 'pt_'||id FROM posts` on every restart silently creates duplicates if the app's runtime ID (e.g. `NewID("pt")`) uses a different scheme. Single-platform posts become cross-platform; Instagram posts get double-published. Run the backfill once, then remove it.
