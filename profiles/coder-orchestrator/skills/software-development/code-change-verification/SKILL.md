...
If a delegated coding CLI exits before producing a handoff, first prove whether it changed the checkout. When the tree is unchanged, continue the same milestone directly: preserve the established RED test, implement toward GREEN, and keep the original verification gates. Do not turn the delegation failure into repeated progress-only final responses.

For direct fallback implementation, prefer small native `patch` edits over generating one large quote-heavy source patch inside Python/`execute_code`. Embedded JavaScript/TypeScript template literals, regexes, and nested quotes make the wrapper itself a second failure surface. Split schema, authorization, route, and UI edits into independently syntax-checked patches; use scripted editing only when real iteration or data transformation justifies it.
...
## External-provider readiness before deployment

When acceptance requires a live third-party interaction (social discovery, payments, email, storage, or messaging), preflight the **effective runtime credential** before replacing binaries or publishing frontend assets:

1. Trace configuration through env loading, startup wiring, and any database-backed integration record. A key in `.env` is not proof the running service receives it; inspect only presence, length, or a redacted fingerprint—never print the secret.
2. Make the smallest authenticated, non-destructive provider-backed request through the exact public route. A locally mocked provider regression proves contract handling, not live-provider readiness.
3. If the credential is absent or rejected, keep deployment and commit/push pending when the user's completion rule requires successful authenticated public E2E. Report the exact blocked boundary instead of deploying first and discovering it in the final browser run.
4. Never weaken the feature to accept typed or client-supplied identity data when provider revalidation is unavailable. The safe failure is to create nothing.
5. If deployment is explicitly required despite the blocker, separate deployed-artifact evidence from the pending provider E2E gate and preserve rollback/database-backup evidence.
...
For menu-specific free/paid modifiers, use `references/versioned-menu-variation-integrity.md`. It covers immutable versioned choices, sale-time snapshots, cross-version cart merge identity, complete add/remove authoring, and Kitchen/history disclosure.
...
For isolated SPA previews, also read `references/spa-preview-prefix-and-browser-evidence.md`. It covers emitted-prefix discovery, public asset/MIME probes, the boundary between transport checks and browser-render evidence, screenshot validation, and staging untracked files. When a static subpath SPA also gains a same-origin backend, read `references/subpath-api-auth-deployment.md` for document-base API resolution, nginx precedence, listener ownership, logout invalidation, and browser network proof. For Vite/React/SvelteKit SPAs deployed beneath an nginx subpath, also read `references/static-spa-subpath-deployment.md`; it covers explicit build bases, emitted HTML checks, prefixed asset probes, browser harness portability, and why importing Vite config into jsdom is the wrong regression boundary. Treat the deployment-base environment as part of the build command, not ambient shell state: repeat it on every rebuild, including the final post-format/post-test build (for example, `BASE_PATH=/projects/app npm run build`). Before rsync, require generated `index.html` asset URLs to carry the prefix; after publication, open a nested public route in a real browser and fail on root-level `/_app` or `/assets` requests. A successful default-base build can overwrite a previously correct subpath artifact, so build success alone never authorizes publication. When one SPA also deploys to a root-domain provider such as Vercel, read `references/dual-target-vite-spa-deployment.md`; align the emitted asset base, client router basename, and host fallback rewrite, build both modes, test direct routes, and inspect the actual provider's commit status rather than assuming GitHub Actions owns deployment. Add the explicit preview web-server location before browser verification: a production catch-all can serve the preview file while injecting conflicting production basename/API globals, so inspect the final public HTML for duplicate runtime injections and require the rendered route—not HTTP 200—to confirm routing. For typography work, pair the screenshot with public-page computed `font-family` and loaded-font checks because appearance alone cannot prove the exact face. When the preview includes uploaded media, attendee avatars, or checkout, also read `references/subpath-preview-media-and-checkout.md` for preview-aware `/uploads/` routing, end-to-end profile-picture contract checks, explicit test-payment finalization, and the Vite build-base trap. If a previously verified preview appears to regress, read `references/preview-mount-persistence-and-checkpointing.md` before changing application code: prove the public URL still serves the isolated basename, API base, asset prefix, Nginx mount, and filesystem artifact. That reference also defines checkpoint-commit handling for long-lived preview worktrees.
...
### Distinguish authentication from client-side role selection

Before promising or reporting authenticated public E2E, trace the application's actual session bootstrap: login/OTP/password flow, server-issued cookie or token, authenticated session lookup, and authorization boundary. A role dropdown, role-valued local storage, direct privileged URL, or client-side workspace switch proves only role-scoped UI behavior; it is not authentication—even when the rendered role is admin.

Preflight this before delegated implementation. If the product has no authentication mechanism, report the acceptance mismatch before launch and ask whether role-scoped public E2E is acceptable or authentication is in scope. Do not wait until the final report, and do not silently downgrade “authenticated public E2E” to role selection. When role-scoped testing is explicitly accepted, label it exactly that and still exercise each relevant role through the public browser.
...
- Do not chain build, push, static deployment, and live-marker checks behind `&&` in one opaque command. A final selector/`grep` miss can return non-zero after the push and copy already succeeded, obscuring which side effects occurred. Run or report separate boundaries: immutable commit contained by the remote default branch; final `index.html` copied to the discovered document root; public HTML names the expected entry asset; changed lazy route chunk resolved and contains a stable semantic marker; exact browser behavior remains a separate E2E gate.
