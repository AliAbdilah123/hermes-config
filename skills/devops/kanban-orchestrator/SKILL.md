...
**Step 0a: verify host tooling before browser reconnaissance.**

If browser/headless tooling is unavailable on the host (common on ARM64 without a Chrome for Testing build), do **not** get blocked on visual browsing. Treat that as a transient environment constraint and pivot instead of retrying.
Examples of fallback behavior:
- Convert the visual task to a conversation task: ask the user to paste or recap the content.
- For a video or media link, ask for the canonical source URL (e.g., YouTube) and use a transcript-style flow.
- For an app idea or design reference, ask the user to describe the core interaction or desired states.
The fallback must preserve intent; do not invent missing details.

**Step 0b: discover available profiles before planning.**
...
**Do not spawn full implementations before constraints are confirmed.** Even when the user references an external system (a repo, URL, post, or workflow sketch), do not begin writing files/branches/cards until the source constraints are actually known. The right move is:
1) try to source the truth (URL, post text, transcript, or user summary)
2) confirm constraints (stack, profiles, env, tools)
3) only then decompose. Generating code/specs/schemas before constraint discovery causes rework and increases context waste.

**Do not loop on unavailable browser tooling.** If headless browser launch fails on the current host (common on ARM64), follow the Step 0a pivot rules instead of retrying. Two failed installation attempts is enough; on the second failure, stop and switch to asking the user for text/recap/source URL.
