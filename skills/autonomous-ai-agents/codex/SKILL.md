...
When you need to diagnose local Codex state (auth mode, installed version, login
status) without entering an interactive session:
...
Local auth file: `~/.codex/auth.json`. Relevant fields:
- `auth_mode` — e.g. `chatgpt` or `api`
- `OPENAI_API_KEY` — present when auth was configured via API key
- `tokens` — present when using OAuth / ChatGPT login
- `last_refresh`

If `codex` is invoked from a service account such as `www-data`, and `$HOME`
points to a directory that isn't writable by that account, Codex may emit a
startup warning:
...
This is non-fatal. If the message pollutes logs or script output, either make
the parent directories searchable, or wrap the binary in a tiny script that
redirects stderr (e.g. `exec /home/ubuntu/.local/bin/codex "$@" 2>/dev/null`).
