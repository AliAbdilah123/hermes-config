...
## Prefixed project API proxy checklist

When the frontend loads but `/projects/<projectName>/api/v1/*` returns 404:

1. Ensure the backend is running and listening on a localhost port.
2. Add an nginx proxy block for the prefixed API path, with matching trailing slashes.
3. Reload nginx and validate the prefixed API path before confirming success.
